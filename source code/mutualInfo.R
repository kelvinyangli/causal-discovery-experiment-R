# compute mutual information b/w variables x and y from their marginal and joint distributions
# the marginal and joint distributions are propagated by junction tree algorithm from gRain package
# there may be computer memory issues when the network is too large or the number of parents is a large value
mutualInfo = function(x, y, junction) {
  
  # joint distributions of x and y
  xyJoint = querygrain(junction, nodes = c(y, x), type = "joint")
  
  # re-order xyJoint according to the order of c(y, x)
  xyJoint = aperm(xyJoint, c(x, y))
  
  # marginalise probX and probY from xyJoint 
  yJoint = apply(xyJoint, y, sum)
  xMarginal = apply(xyJoint, x, sum)
