...
prediction = function(cptsLearned, dataTestingNumeric, maxNumParents, maxNumValues, debug = FALSE) {
  ...
  for (i in 1:nrow(dataTestingNumeric)) {
    ...
    if (debug) {
      #cat("computing row", i, "\n")
      if (i == 0.2*nrow(dataTestingNumeric)) cat("20% is done \n")
      if (i == 0.4*nrow(dataTestingNumeric)) cat("40% is done \n")
      if (i == 0.6*nrow(dataTestingNumeric)) cat("60% is done \n")
      if (i == 0.8*nrow(dataTestingNumeric)) cat("80% is done \n")
      if (i == nrow(dataTestingNumeric)) cat("100% is done \n")
    }
    
    for (ii in 1:numNodes) { # compute rmse and bir for all variables for ith data point
      
      #if (debug) cat("computing for row", i, "node", ii, "\n")
...
#######################################################################################################
autoPrediction = function(currentDirectory, numIterations, maxNumParents, maxNumValues,
                          learningMethod, debug = FALSE) {
  ...
  # read in the right slurm job parameter. NOTE. WE msut have the correct number of parameters or we will miss file
  # ${SLURM_ARRAY_TASK_ID}
  ...
  #filename = allDataSets[fileIndex]
  ...
  DIR <- paste0("/home/kli/p2016040001/Experiments_31032016/", currentDirectory, "/Evaluations/")
  
  # read learned cpts
  cptsLearned = readRDS(paste0(currentDirectory, "/Learned networks/CPTs/", learningMethod, "/", learnedCPTs[fileIndex]))
  ...
  write.csv(actual$rmse, paste0(DIR, "rmse/", learningMethod, "/", allDataSets[fileIndex], ".csv"), row.names = FALSE)
  
  write.csv(actual$bir, paste0(DIR, "bir/", learningMethod, "/", allDataSets[fileIndex], ".csv"), row.names = FALSE)
